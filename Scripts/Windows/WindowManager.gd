...
signal window_opened(window: AppWindow, program_data: ProgramData)
...
func _get_centered_position(window: AppWindow) -> Vector2:
	...
	var layer_size := window_layer.size
...
func get_windows_above(reference_window: AppWindow) -> Array[AppWindow]:
	...
	for child in window_layer.get_children():
		var candidate := child as AppWindow
...
func _is_window_above(candidate: AppWindow, reference_window: AppWindow) -> bool:
